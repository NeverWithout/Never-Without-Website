...
function staff_directory($param = null){
	...
	if((isset($id) && $id != '') && (!isset($cat) || $cat == '')){
		...
		if($staff->linkedin != ''){
			$linkedin = "<a target='_blank' href=\"$staff->linkedin\"><img src='/wp-content/plugins/staff-directory/images/linkedin.jpg' /></a>";
		}else{
  		$linkedin = '';
  	}
		$accepted_single_tags = array("[name]", "[photo_url]", "[position]", "[email]", "[phone]", "[bio]", "[linkedin]", "[category]");
		$replace_single_values = array($staff->name, $photo_url, $staff->position, $staff->email_address, $staff->phone_number, $staff->bio, $staff->linkedin, $staff->category);

		$accepted_formatted_tags = array("[name_header]", "[photo]", "[email_link]", "[bio_paragraph]", "[linkedin]");
		$replace_formatted_values = array("<h3>$staff->name</h3>", $photo, "<a href=\"mailto:$staff->email_address\">Email $staff->name</a>", "<p>$staff->bio</p>", $linkedin);
	...
	if((isset($cat) && $cat != '') && (!isset($id) || $id == '')){
		...
		foreach($staff as $staff){
  		...
  		if($staff->linkedin != ''){
			...
			$accepted_single_tags = array("[name]", "[photo_url]", "[position]", "[email]", "[phone]", "[bio]", "[linkedin]", "[category]");
			$replace_single_values = array($staff->name, $photo_url, $staff->position, $staff->email_address, $staff->phone_number, $staff->bio, $linkedin, $staff->category);
