...
abstract class dt_structure {
	...
	**/
	...
	public function __construct($data_src, $tmp_trans_type='qry')
	{
        if (array_key_exists($data_src, $_SESSION)) {
			...
			$this->disp_dt = '[' . strtoupper($this->data_type) . ']';
			$this->stmt = false;
	...
	**/
	...
	//*************************************************************************
	public function assoc_result() { return $this->result; }
	...
	**/
	//*************************************************************************
    public function clear_result() { $this->result = array(); }
	...
	**/
	...
	public function bind($user, $pass)
	{
		...
		switch ($this->data_type) {
			...
			case 'sqlite':
			case 'db2':
				if ($this->result[0][$_SESSION['auth_pass_field']] == $pass) {
					$ret_val = true;
					$this->bound = true;
				}
				break;
