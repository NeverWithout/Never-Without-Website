...
* @access		private
* @version 		Started/Copied: 12-23-2011 updated: 3-22-2012
*/
...
class dt_sqlsrv extends dt_structure
{
	...
	**/
    ...
    public function close()
	{
		...
		if (!$this->reuse_connection) {
			if ($this->handle) { return sqlsrv_close($this->handle); }
		}
		return true;
	}
	...
	**/
	...
	public function query($query, $params=false)
	{
		...
		// Execute Query
		if ($params) { $this->stmt = sqlsrv_query($this->handle, $query, $params, array('Scrollable' => SQLSRV_CURSOR_STATIC)); }
		else { $this->stmt = sqlsrv_query($this->handle, $query); }
	...
	**/
	...
	private function print_error($error=false)
	{
		if (!$error) { return false; }
			...
			$error_msg = $msg_break;
			if (!is_array($error)) { $error_msg .= '[Error] -> ' . $error . $msg_break; }
			else {
				$error_msg .= '[Error] -> ';
				ob_start();
				print "<pre>\n";
				print_r($error);
				print "</pre>\n";
				$error_msg .= ob_get_clean() . $msg_break;
			}
	...
	**/
    ...
    private function _prepare($stmt=false)
    {
    	...
    	if ($this->stmt) { sqlsrv_free_stmt($this->stmt); }
