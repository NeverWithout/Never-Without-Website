...
* @version 		Started: 9-20-2005 updated: 3-21-2012
*/
...
class data_trans {
	...
	**/
	...
	public function __construct($data_src='', $tmp_trans_type='qry')
	{
        ...
        // Data Library
        $data_lib = dirname(__FILE__) . '/lib/';
        
        // Include (dt_structure) class
        $data_structure_lib = $data_lib . 'dt_structure.class.php';
        
        if (file_exists($data_structure_lib)) {
            include_once($data_structure_lib);
        }
        else {
            trigger_error("Error: $data_structure_lib does not exist!!\n");
        }
        
        // Include necessary Data Object Library
        $data_object_lib = $data_lib . 'dt_' . $this->data_type . '.class.php';
        
        if (file_exists($data_object_lib)) {
            include_once($data_object_lib);
        }
        else {
            trigger_error("Error: $data_object_lib does not exist!!\n");
        }
			
        // Create new Data Object
        $new_obj_code = "\$this->data_object = new dt_" . $this->data_type . "(\$data_src, \$tmp_trans_type); return \$this->data_object;";
        $this->data_object = eval($new_obj_code);
	}
