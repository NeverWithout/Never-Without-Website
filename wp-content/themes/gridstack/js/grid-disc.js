  ...
  var Grid = (function ($) {
  	...
  	function initEvents() {
        ...
        jQuery('ul.og-grid li a').css('opacity', '1');
  			...
  			// if cursor is not default preview item
  			if ( jQuery(this).css('cursor') != 'default' ) {
                    
  				var $item = $( this ).parent();
  				// check if item already opened
  				current === $item.index() ? hidePreview() : showPreview( $item );
          jQuery('ul.og-grid li a').css('opacity', '.5');
  	...
  	Preview.prototype = {
        ...
        this.$bulletpoints = $( '<div></div>' );     
        this.$rightcolumn = $('<div class="bullet-points"></div>').append( this.$bulletpoints );
        this.$info = $('<div class="about"></div>').append(this.$title, this.$description)
  			this.$details = $( '<div class="og-details"></div>' ).append( this.$info, this.$rightcolumn );
  		...
  		update : function( $item ) {
            ...
            bulletpoints : $itemEl.data('bulletpoints'),
  				};

  			this.$title.html( eldata.title );
  			this.$description.html( eldata.description );
        this.$bulletpoints.html( eldata.bulletpoints);
